...
public class BugsCommentsModelTests : IClassFixture<TestFixture>
{
    ...
    public async Task BugsCommentsModel_AddCommentToBug()
    {
        ...
        await bcm.DeleteBugComment(_comment.Id);
    }

    [Fact]
    public async Task BugsCommentsModel_GetCommentsToBug()
    {
        var bug = await db.Bugs.Where(x => x.Name == "Bug 2").FirstOrDefaultAsync();
        if (bug == null) throw new Exception("Bug not found");
        var bugById = await bm.GetBugByIdAsync(bug.Id);
    ...
    public async Task BugsCommentsModel_DeleteCommentsToBug()
    {
        ...
        if (bug == null) throw new Exception("Bug not found");
        var bugById = await bm.GetBugByIdAsync(bug.Id);
        var initialComments = await bcm.GetBugsComments(bugById.Id);
        var initialCount = initialComments.Count;

        var comment = new BugsComments { Comment = _comment.Comment };
        var comment1 = new BugsComments { Comment = _comment1.Comment };

        await bcm.AddBugComment(bugById.Id, comment);
        await bcm.AddBugComment(bugById.Id, comment1);

        var afterAddComments = await bcm.GetBugsComments(bugById.Id);
        Assert.Equal(initialCount + 2, afterAddComments.Count);

        await bcm.DeleteBugComment(afterAddComments[0].Id);

        var afterDeleteComments = await bcm.GetBugsComments(bugById.Id);
        Assert.Equal(initialCount + 1, afterDeleteComments.Count);
    }
